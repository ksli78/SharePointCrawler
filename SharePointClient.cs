...
public class SharePointClient : IDisposable
{
    ...
    /// <summary>
    /// Retrieves the REST endpoints for all document libraries under the
    /// root SharePoint site.
    /// </summary>
    public async Task<List<string>> GetDocumentLibraryUrlsAsync()
    {
        var libraries = new List<string>();
        var endpoint = $"{_siteUrl}/_api/web/lists?$filter=BaseTemplate eq 101&$select=RootFolder/ServerRelativeUrl&$expand=RootFolder";
        using var response = await _client.GetAsync(endpoint).ConfigureAwait(false);
        if (!response.IsSuccessStatusCode) return libraries;

        using var stream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false);
        using var document = await JsonDocument.ParseAsync(stream).ConfigureAwait(false);
        JsonElement root = document.RootElement;
        if (root.TryGetProperty("d", out var d)) root = d;
        if (root.TryGetProperty("results", out var results)) root = results;
        else if (root.TryGetProperty("value", out var value)) root = value;

        if (root.ValueKind == JsonValueKind.Array)
        {
            foreach (var item in root.EnumerateArray())
            {
                if (item.TryGetProperty("RootFolder", out var rf) && rf.TryGetProperty("ServerRelativeUrl", out var urlProp))
                {
                    var url = urlProp.GetString();
                    if (!string.IsNullOrWhiteSpace(url))
                    {
                        var libUrl = $"{_siteUrl}/_api/web/GetFolderByServerRelativeUrl('{url}')?$expand=Folders,Files";
                        _writer.WriteLine($"Discovered library: {libUrl}");
                        libraries.Add(libUrl);
                    }
                }
            }
        }
    ...
    public async IAsyncEnumerable<DocumentInfo> GetDocumentsAsync(string libraryRelativeUrl)
    {
        ...
        _writer.WriteLine($"Scanning library: {libraryRelativeUrl}");
                    ...
                    if (fileArray.ValueKind == JsonValueKind.Array)
                    {
                        foreach (var fileElement in fileArray.EnumerateArray())
                        {
                            ...
                            try
                            {
                                ...
                                _writer.WriteLine($"Processing document: {docInfo.Name}, Url: {docInfo.Url}");
