...
public class SharePointClient : IDisposable
{
    ...
    protected async Task SendToExternalApiAsync(DocumentInfo doc)
    {
        ...
        // Prepare values to mirror your curl fields
        var docId = doc.Metadata.TryGetValue("UniqueId", out var id) ? id?.ToString() : "Test";
        var etag = doc.Metadata.TryGetValue("ETag", out var et) ? et?.ToString() : "12345";
        var lastModified = doc.Metadata.TryGetValue("Modified", out var mod) ? mod?.ToString() : DateTime.UtcNow.ToString("yyyy-MM-ddTHH:mm:ssZ");
        var title = doc.Metadata.TryGetValue("Title", out var ttl) ? ttl?.ToString() : Path.GetFileNameWithoutExtension(doc.Name);
    ...
    var url = "http://adam.amentumspacemissions.com:8000/ingest/upload";
        ...
        // File part (field name must be "file")
        ...
        form.Add(new StringContent(etag), "etag");
        form.Add(new StringContent(lastModified), "last_modified"); // Prefer ISO-8601 as shown above
